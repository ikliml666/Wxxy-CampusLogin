...
│ IPC 层  hooks/tauriApi.ts（60 invoke + 16 事件订阅，唯一出口）                            │
└─────────────────────────────┬───────────────────────────────────────────────────────────┘
...
| 配置模型 | `config/{model,persist,validate,schedule}.rs`（跨平台，但 `persist` 依赖 DPAPI，安卓不用；`schedule` 是定时动作纯函数） | `config_state.rs`（`Settings` 37 字段 + Keystore 桥 + 迁移，当前 `config_schema_version` 4：v3→v4 新增 `background_check_idle_interval` 默认 300000 与 `scheduled_login/scheduled_logout_minutes`（0=禁用）） |
...
[1] 组件触发
    components/layout/DockNav.tsx:415 取 useAuthStore 的 doLogin，绑定到按钮 onAction
      → hooks/useAuthStore.ts:145-204 doLogin
          先 saveConfigDirect(loginConfig)（:157）落盘，再

[2] 出 IPC
    hooks/tauriApi.ts:152  doLogin: (adapterName) => invoke<LoginResult>('do_login', { adapterName })

[3] 命令入口（并发门）
    app/startup.rs:69 注册的 crate::commands::login::do_login
      → commands/login.rs:56 do_login
          :57-59 清残留自动退出倒计时 → :66 tasks.is_logging_in.try_acquire()
          → :73 auth::service::full_login(&state, &app_handle, adapter)

[4] 业务编排
    auth/service.rs:20 full_login
      :21-24 读配置并校验 user/password → :31-38 get_adapters_cached / wait_for_adapter(10000)
      → :43 ensure_ethernet_ip_for_login → :47-51 get_adapters_force
...
[5] 协议执行
    auth/session.rs:87 预检 auth/portal.rs:95 check_portal_full（只读 GET，无凭据）
      → auth/session.rs:111 auth/protocol.rs:181 do_login_with_retry(..., 3, ...)
           → auth/protocol.rs:124 do_login_request
                :125-127 config/validate.rs:10/23/31 三处凭据校验
                :129 network/client.rs:8 PORTAL_URL.load() + portal.rs:42 ensure_portal_port
                :147 network/client.rs:91 create_safe_http_client(15s, local_addr)
                → HTTP GET /eportal/portal/login（reqwest，无代理）
                :142-143 脱敏后打日志；:154 auth/portal.rs:69 redact_credentials
                → 限长读体（1MB）→ protocol.rs:216 parse_login_result
...
[6] 结果回流与副作用
    auth/session.rs:119-173 复核（parse_error→查 Portal；"已经在线"但探测不通→降级失败）
      ...
      → auth/service.rs:215 post_login_handler
...
[7] 回到 UI
    infra/events.rs:18 app_handle.emit('login-log', payload)
      → hooks/tauriApi.ts:169 createEventListener<...>('login-log')
        → hooks/useEventListeners.ts:267 useLogToastStore.addLog(message, type)
          → shared/LogPanel / RightPanel / ToastContainer 渲染
    do_login 的 Promise 同时 resolve CommandResult → useAuthStore 写 status/isLoggingIn（:165-202）
      → 详情面板与状态条经 zustand 订阅重渲染
```

安卓端同一动作的差异点：`invoke('do_login')` → `protocol_cmds.rs:10` 先 `ensure_wifi_bound`（`protocol_cmds.rs:220`，绑定成功且路径非 `already_bound` 时清 HTTP 连接池 `protocol_cmds.rs:232-236`）→ `run_login`（`protocol_cmds.rs:60`）→ 同一个 `auth::protocol::do_login_with_retry` → `login_history::append`（`login_history.rs:51`）→ `app.emit("login-log")`（`monitor_loop.rs:139`）。

### 一次后台巡检拍（桌面端）

```text
monitor/background_task.rs:29-49 循环（每轮重读 config.background_check_interval.max(10000)）
  → monitor/background_check.rs:342 run_background_check（spawn_blocking）
      → :15-337 run_background_check_blocking
          :19 is_checking.try_acquire（单飞）→ :29-38 取适配器
          → monitor/campus_check.rs:33 check_campus_network（SSID/有线 profile/子网/网关）
          → :144-178 Portal 探测（双适配器并行）→ auth/portal.rs:95 check_portal_full
          → :182-207 请求失败计数（阈值 5 → MAC 重置）
          ...
          → monitor/auto_auth.rs:29 try_auto_login_on_preparation（冷却 → 熔断 → 登录锁 → auth::service.rs:20 full_login）
...
| [[desktop-commands]] | 桌面 56→60 条命令的逐条参数、并发门与 JSON 返回形状 |
...
| [[android-backend]] | 安卓后端 48 条命令、15 个模块（2026-09-13 新增 `quality_history`）、平台探针与监控状态机 |
...
3. **配置字段集是"交集 + 双向差集"**：桌面 `Config` 46 字段（`config/model.rs:10-110`）、安卓 `Settings` 37 字段（`android/src-tauri/src/config_state.rs:15-76`），共有 33（2026-09-13 双端同加 `scheduled_login/scheduled_logout_minutes` 后由 31 增至 33）；而两套前端类型又各自是第三份定义（桌面 `settings/types.ts:3-55` 45 字段、安卓 `settings/types.ts:3-56` 45 字段，两者均指各自 `Config` 接口本体的字段总数）。新增字段无法照抄任何一端。
4. **默认值分叉已实际发生**：`background_check_interval` 桌面 15000（`config/model.rs:186`）与安卓 60000（`android/src-tauri/src/config_state.rs:92`）不一致，且安卓迁移会把 15000 强刷为 60000（`config_state.rs:250`）；`default_panel` 桌面空串（`model.rs:195`）与安卓 `"dashboard"`（`config_state.rs:101`）不同。桌面 `validate.rs` 的 clamp 下限 10000（`config/validate.rs:103`）对不上安卓的 5000。**配置 schema 版本号也各走各的**：安卓前端 `android/frontend/src/settings/constants.ts:54` 的 `DEFAULT_CONFIG.configVersion` 曾照抄桌面的 2，2026-09-13 改为对齐安卓后端 `config_schema_version`（现 4，`config_state.rs:127`，v3→v4 新增 `background_check_idle_interval` 与 `scheduled_login/scheduled_logout_minutes`）；桌面同名 `configVersion` 仍是 2（`tauri-app/src-tauri/src/config/model.rs:216`、`tauri-app/frontend/src/settings/constants.ts:51`）。两端字段名近义（`config_version` ↔ `config_schema_version`）但语义独立，跨端对照时不可互推。
...
13. **两端命令名有三处刻意不一致**（由前端 `tauriApi` 转调抹平）：`verify_windows_identity` ↔ `verify_biometric_identity`、`get/set_auto_launch` ↔ `get/set_boot_autostart`、`get_config` 返回 `Config` ↔ 返回掩码后的 `serde_json::Value`（`android/src-tauri/src/config_state.rs:304-317`）。任何按"命令名逐字相同"假设写的自动化核对都会在这三处误报。
14. **文档计数漂移（2026-09-13 两轮核对定稿）**：`modules/desktop-commands.md` 原称安卓注册 45 项、行号 `lib.rs:54-100`——该数字对应**电池白名单命令面落地前**的旧状态（`battery_cmds` 三命令由 main 提交 `6cbee74` 引入）。以主仓库工作区实测，安卓命令面为 **48 项**：注册块 `android/src-tauri/src/lib.rs:57-106`（`:57` = `.invoke_handler(tauri::generate_handler![`，`:58-105` = 48 条命令条目，`:106` = `])`），与桌面 `app/startup.rs:63-124`（条目 `64-123`，60 条）引用口径一致；核实命令 `grep -cE '^\s*[a-z_0-9]+::[a-z_0-9]+,$' android/src-tauri/src/lib.rs` → 48。**计数必须在主仓库工作区 `E:\ik\Documents\trae_projects\1\Wxxy-CampusLogin` 上测**：滞后于 main 的分支/worktree 缺 `battery_cmds.rs`，会误测成 45 项/`54-100`（同日曾有一轮据此想把 48 改回 45，已被实况驳回）；`modules/android-backend.md` 原称 `lib.rs:10-23` 声明 15 个模块，实际 14 个（`campus_detect`/`android_state`/`config_state`/`cpu_affinity`/`identity_gate`/`login_history`/`protocol_cmds`/`self_service_cmds`/`monitor_loop`/`account_cmds`/`system_cmds`/`quality_cmds`/`update_cmds`/`battery_cmds`）。两处均已改为实计数。（2026-09-13 追记：本轮新增 `quality_history` 后模块数为 15，声明块 `lib.rs:11-25`。）
15. **`default_panel`"Rust 侧零引用"的说法不准确**：`grep -rn default_panel tauri-app/src-tauri/src/` 命中 3 处，除 `config/model.rs:61/195` 外还有 `network/adapter.rs:255`（测试辅助 `make_test_config`）。`self_reverify_each_action` 同因命中 `network/adapter.rs:235`（同为测试辅助）。结论（"仅前端实际消费"）成立，但证据描述需修正（`modules/desktop-config.md` 的 `default_panel` 条目已于 2026-09-13 按此改写证据）。
