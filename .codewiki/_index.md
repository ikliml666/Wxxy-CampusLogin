...
- [[decisions\night-outbound-switch|夜间出站自动切换（出站层夜切）]]
...
- [[decisions\android-notification-permission-prompt|安卓通知权限主动索取链路（13+ 弹框 / 13- 与国产 ROM 跳设置页）]]
...
- [[decisions\lightweight-mode-desktop|桌面轻量化模式：销毁 WebView、EcoQoS 与退出守卫]]
...
- [[decisions\night-operator-switch|自动切换运营商（原晚间断网自动切换）]]
- [[decisions\query-bind-status-no-gate-by-design|自助服务查询命令的"无验证门 + 密码逐个回退"是有意设计]]
- [[decisions\account-display-name-id-separation|账号显示名与内部 id 分离（含自动建号契约）]]
...
- [[learnings\binding-operator-select-excludes-campus|「运营商下拉里没有无锡学院」先分清是哪个 Select]]
...
- [[learnings\account-switch-ui-state-desync|切换账号后 UI 不刷新——三条路径全部断链的静默契约断裂]]
