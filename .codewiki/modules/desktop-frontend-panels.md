...
- `formatSpeed`（`NetworkPanel.tsx:41-45` 附近，bit/s → Mbps / Kbps）。
- 主组件（`NetworkPanel.tsx:50-767`，2026-09-22 起含夜间出站切换卡）：`disabledAdapters`（`NetworkPanel.tsx:49`）、`config` 浅比较订阅（`NetworkPanel.tsx:52`）、`dohEnabling` / `dnsResetting` / `gettingNewIpAdapter` / `enablingAdapter` 四个单槽位忙态（`NetworkPanel.tsx:53-56`，`dnsResetting` 为重置 DNS 新增）、`ipc = tauriApiWithRetry`（`NetworkPanel.tsx:57`）、`mountedRef`（`NetworkPanel.tsx:58-66`）、`dnsStatus` / `dnsChecking`（`NetworkPanel.tsx:67-68`）、`refreshAdapters` / `isRefreshingAdapters`（`NetworkPanel.tsx:69-70`）、`dnsFamily` 与 `familyTabs`（`NetworkPanel.tsx:72-77`）。
- 行为：`handleCheckDns`（`NetworkPanel.tsx:103`，`checkDnsDohStatus` + 两类 warning 日志）；`handleSetupDnsDoh`（`NetworkPanel.tsx:126`，`setupDnsDoh(dnsFamily)`，随后独立 try 刷新状态，避免失败误报）；`handleResetDns`（`NetworkPanel.tsx:152`，`resetDns()` 恢复默认 DNS，同样独立 try 刷新状态；补齐一键设 DNS 后的可逆性）；`handleGetNewIpForAdapter`（`NetworkPanel.tsx:178`，`dhcpReleaseRenewAdapter` + `announceDhcpResults` + `refreshAdapterData()`）；`handleEnableAdapter`（`NetworkPanel.tsx:195`，`enableAdapter` + 带 `includeDisabled` 刷新）；`getDnsQuality`（`NetworkPanel.tsx:215`，四级 `excellent`/`good`/`basic`/`none`）。
- 视图：网卡卡（空态、主/副置顶排序、状态徽标与启用/获取新 IP/刷新 DHCP 按钮）；适配器设置卡（主适配器选择、**主适配器"指定账号"下拉**（哨兵 `FOLLOW_CURRENT_ACCOUNT = '__follow__'`，Radix Select 不允许空串 value，落盘时空串；选项为 `AccountItem` 列表）、副适配器选择与 `dualAdapter` 联动、**副适配器"指定账号"下拉**（仅已选副适配器时显示）、说明文案）；DNS 优化卡（检测/优化/重置 DNS 按钮、IP 族切换、未检测/检测中态、逐适配器明细与 profile 级 DNS、DoH 不支持提示）；**夜间出站切换卡**（`NetworkPanel.tsx:374-441`，2026-09-22 新增，见 [[night-outbound-switch]]）：开关 `enableNightOutboundSwitch`（默认关）+ 时间说明与管理员权限提示（`:439`）+ 网卡排序列表——`outboundOrder`（`:61-64`）以 `outboundPriority` 为基座、未列入的适配器按发现顺序追加尾部，每张卡 ↑/↓ 按钮（`:415-433`，不引入拖拽避免安卓 WebView 滚动冲突）落盘完整顺序列表，**排第一的卡常显「夜间出站目标」徽标**（`:406-409`，不受开关影响）。适配器绑定账号的解析语义见 [[adapter-account-binding]]。
