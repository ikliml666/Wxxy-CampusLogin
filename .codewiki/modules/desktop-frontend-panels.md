...
- props `AccountPanelProps`（`AccountPanel.tsx:35-45`，2026-09 新增 `onRenameAccount`）。
...
- 视图：网卡卡（`NetworkPanel.tsx:219-347`，空态、主/副置顶排序、状态徽标与启用/获取新 IP/刷新 DHCP 按钮）；适配器设置卡（`NetworkPanel.tsx:351-476`，主适配器选择、**主适配器"指定账号"下拉 `NetworkPanel.tsx:399-413`**（哨兵 `FOLLOW_CURRENT_ACCOUNT = '__follow__'` `:40`，Radix Select 不允许空串 value，落盘时空串；选项为 `AccountItem` 列表）、副适配器选择与 `dualAdapter` 联动、**副适配器"指定账号"下拉 `NetworkPanel.tsx:455-470`**（仅已选副适配器时显示）、说明文案 `:474`）；DNS 优化卡（`NetworkPanel.tsx:480-669`，检测/优化/重置 DNS 按钮、IP 族切换、未检测/检测中态、逐适配器明细与 profile 级 DNS、DoH 不支持提示）。适配器绑定账号的解析语义见 [[adapter-account-binding]]。
