...
| `requiredNetworkName` | `string` | 校园网 SSID（默认 `i-wxxy`；默认值下额外匹配 `iwxxy-2`/`iwxxy-3`，匹配逻辑在桌面 `campus_check.rs`，安卓无 SSID 检测通道） |
