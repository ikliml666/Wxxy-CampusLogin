...
新增面板默认静态导入，除非确属低频对话框。注意 `App.tsx:20` 是静态导入的 `LogPanel`，而 `App.tsx:29` 注释仍称"仅低频的 LogPanel/对话框保留懒加载"、`App.tsx:46-48` 注释记录了 LogPanel 从 `lazy` 回归静态导入的实测原因——注释与实现已不一致（以静态导入为准）。
