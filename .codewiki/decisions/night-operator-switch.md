...
校园网（无锡学院）的运营商线路有固定断网规律：周日、周一 23:00 与周五、周六 23:30 之后，电信/移动/联通服务下线无法上网，需把登录运营商切到"无锡学院"（`config.operator = ''`）才能继续上网；次日早晨线路恢复后应切回原运营商。2026-09-19 与需求方确认形态为**自动定时切换**：一个开关（默认关），开启后到点自动切换，无需手动。
...
- 切换时刻：weekday（`num_days_from_sunday()`，0=周日）0/1 → 1380 分钟（23:00），5/6 → 1410 分钟（23:30），周二~周四无。
...
- **桌面 `autoExitAfterLogin` 默认 true**：夜切登录成功即自动退出，恢复被推迟到用户下次启动 App（启动后 30s 内补恢复）。i18n 描述文案已写明"需保持应用在后台运行"。
