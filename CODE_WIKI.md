...
> **版本**: v2.3.6 | **架构**: Tauri 2 (Rust 后端 + React/TypeScript 前端)
...
| `campusCheckStartMinutes` | u16 | 460 | 校园网检测时段起点（分钟数，460=7:40，0=禁用门控），支持旧字段名 `campusCheckStartHour` 反序列化 |
| `campusCheckEndMinutes` | u16 | 0 | 校园网检测时段终点（分钟数，0=不限制；<= 起点时视为仅受起点限制，与 `campusExitEndMinutes` 同退化规则） |
...
| `is_campus_check_silent()` | `campus_check.rs` | 检测静默期判定（start=0 禁用门控；end=0/<=start 退化为仅起点限制）；`background_check` 与 `auto_auth` 共用 |
...
| `MascotFigure.tsx` | 看板娘展示组件（双端同构）。`variant` 对应 `/girl/mascot-{portrait,welcome,empty,celebrate,sponsor,offline,alert,busy,update}.webp`（2026-09-12 新增 alert/busy/update 三变体，见 `assets/ui-girl/original/` 归档），`size` 三档（sm/md/lg）；tailwind 尺寸类无法被 className 可靠覆盖，特殊尺寸场景用裸 img。**背景装饰娘 4 张**（`mascot-bg-{laptop,nap,lounge,tea}.png`）不经组件、低透明度裸 img 直引。**桌面端**：App.tsx 布局层两个 fixed 侧边娘（≥1360px 宽视口显示，2026-09-12 起从 14 张 `side-*` 池按天序号轮转取左右各一张——`pickDailySideMascots` 按 epoch 天数取第 k 个有序组合，连续 182 天（14×13）不重样后循环，同日固定、左右不重复，每分钟比对日期跨天即换，`top-1/2 -translate-y-1/2` 锚定视口两侧垂直居中、滚动恒定可见，w-36、opacity 0.26/暗色 0.16，right 侧避让 w-72 的 RightPanel；桌面默认窗口 1360×768 使立绘默认即可见（常态左右间隙 24/16px），<1360 视口媒体查询自动隐藏；立绘宽 W、断点 B、默认窗口三者联动约束 `B ≥ 320 + 720 + 2×(W+16)` 且 B 取默认窗口宽）；RightPanel 日志栏的茶娘为**运行日志卡内部底部水印**（absolute bottom-3 低透明度，卡容器 relative overflow-hidden，日志条目少时从空白处露出）。**安卓端**：总览/网络质量/设置三面板滚动末尾底部娘保留（窄屏无侧边空间），设置页图加 `pb-72` 撑滚动余量避让固定底栏；2026-09-12 起底部娘与平板侧边娘接入每日轮换（`shared/dailyMascot.ts`：`SIDE_MASCOT_POOL` 14 张 `side-*` 按 UTC 天数取第 `day+offset` 张，offset 错开使同屏多处同日不重复；`useDailyMascots` 每分钟比对日期跨天即换）。平板 TabletShell 两个 fixed 侧边娘（左侧 ≥900px、右侧 ≥1330px 让过 288px 右栏，w-32、opacity 0.22/暗色 0.14）。错误边界 fallback 用 `mascot-alert` 娘图（`MascotVariant` 已扩 `alert`）。**展示性能约定**：装饰娘一律 `loading="lazy"` + 低透明度静态展示、不加动画引擎——解码内存 = 宽×高×4（800×1200 一张 3.84MB），同屏合成层 ≤3-4；Live2D/Lottie 经调研不引入（WebView 内存数百 MB + Cubism 许可约束 + 无现成二次元素材）。**坑：`space-y-*` 容器的子元素 margin-bottom 被 `space-y-4 > * + *` 规则锁死（specificity 更高），间距要用 padding 不用 margin**。素材链：AI 原图归档 `assets/ui-girl/original/` → rembg(isnet-anime) 抠图 → PNG 档案 → 转 WebP（q84 原尺寸，-85%）→ 双端 `public/girl/*.webp`（代码引用 .webp，PNG 不入打包） |
...
*文档版本: v2.3.6 | 基于代码版本: CampusLogin v2.3.6 | 更新日期: 2026-09-12 | 五段手册层（概览/架构速查/关键约定/踩坑记录/决策记录）+ 附录 A~H 参考层；含安卓端详解（附录 A §4.16 / 附录 B §5.14 / 附录 C §6.1.1）；精简原则：已解决缺陷的修复过程叙述与历史版本流水移除，保留架构、约定、踩坑、决策、认证与自助服务协议知识及已知限制*
