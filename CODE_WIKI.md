...
| 启动延迟 10 秒 | latency loop 启动后先 sleep 10s 再开始检测，避免网络未稳定时 HTTPS 延迟异常 |
...
| 改进项 | 说明 |
|--------|------|
| 启动延迟 10 秒 | 循环开始前 `sleep(10s)`，避免网络未稳定时 HTTPS 测试延迟异常 |
...
network/
  ...
  └── quality.rs ← adapter.rs, client.rs, surge-ping, tokio-rustls, timing.rs
      [两阶段检测: DNS/DoH → HTTPS]
...
*文档版本: v2.2.5 | 基于代码版本: CampusLogin v2.2.5 | 更新日期: 2026-06-16*
