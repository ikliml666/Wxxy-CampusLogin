...
| `set_dns_via_api()` | 适配器级 DNS 设置（原有函数，有线适配器使用） |
...
**注销成功后**: 重置 `any_adapter_online`/`last_a1_online`/`last_a2_online`/`has_logged_online` 为 false，`disconnect_reconnect_count` 归零，取消自动退出倒计时，设置 60 秒注销保护期 (`logout_protected_until`)
...
*文档版本: v2.2.5 | 基于代码版本: CampusLogin v2.2.5 | 更新日期: 2026-06-09*
