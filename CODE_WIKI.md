...
- **协议核心单点共享**:android 的 src-tauri 以 Cargo path 依赖 `campus-login`(主仓库 src-tauri),协议层零复制。终态收敛为 git 依赖 + tag。
- **lib.rs 门控后跨平台可见面**:`account / auth / config / infra / network / platform / self_service`;desktop 专属:`app / commands / helper / monitor / update`。auth/mod.rs 内 session/service/dual_adapter_executor 再度 cfg(desktop)。
- **platform/mod.rs 按文件拆分**:`console_output` 跨平台(GBK 解码协议共用),其余(elevation/dns_config/gpu/identity/autostart/helper_spawn)desktop。
- **network/mod.rs re-export 门控**:`dhcp_release_renew_all/single` 与 `enable_adapter` 仅桌面;`dhcp_release_renew_single` 另有非 Windows 存根版(skipped 语义)供 `auth/failure_tracker` 三处调用点跨平台编译。
- **crypto.rs**:非 Windows 下 `encrypt/decrypt` 为 Err 存根(阶段 1 密码不落盘;阶段 2 换 Keystore)。
- **lifecycle.rs** 全局快捷键三处 cfg 门控,非桌面空实现保持调用点不变。
- **capabilities 拆分**:`default.json`(core+notification,跨平台)+ `desktop.json`(`platforms: ["windows","macOS","linux"]`,shell/shortcut/autostart)——否则 tauri-build 在安卓 target 因找不到 shell:allow-open 报错。

### 20.2 关键决策

- **强制走 WLAN**:Kotlin `ConnectivityManager.bindProcessToNetwork()`(经 netd FwmarkServer 对进程全部 socket 含 Rust native 生效),插件 `plugins/network-bind`(手写,基于官方 biometric 插件源码为模板;CLI 脚手架 `plugin new` 在非终端环境不可用)。reqwest 源 IP 绑定为补充手段(campus_detect 缓存 wlan0 IP)。
- **ICMP 不可用**:SELinux 禁 raw socket,surge-ping 失效;校园网检测 = wlan0 源 IP 枚举 + /18 子网(复用桌面 `is_same_subnet_18(ip_str, gateway_str)`,**签名是双 &str**)网关取 `Config::default().campus_gateway` 同源 + Portal TCP 可达替代 ping。
- **蜂窝接口排除**(TDD 测试逼出):`rmnet*`(高通)/`ccmni*`(MTK)永不作为登录源,防止流量走移动数据。
- **登录/注销接线**:`do_login_with_retry(user, password, operator, adapter_ip: Option<&str>, max_retries, is_quitting: &AtomicBool)` 为**同步 fn**,必须 `tauri::async_runtime::spawn_blocking` 调用;返回 Value 已含 success/message 判定,命令层直接转发。注销同样要 user_account。
- **settings**:仅非敏感字段(user/operator/portalUrl),`#[serde(rename_all = "camelCase")]` 对齐前端;**密码不落盘**(阶段 2 接 Keystore/biometry)。

### 20.3 构建环境与坑清单(Windows)

| 坑 | 处置 |
|---|---|
| jniLibs symlink 需要权限 | Windows 必须开启**开发者模式**(AllowDevelopmentWithoutDevLicense=1),否则 tauri android build 卡 so 链接 |
| `tauri android dev` 后台(无 TTY)死循环 "pick a device" | **dev 必须前台跑**;设备参数传**蓝牙名**(`dumpsys bluetooth_manager` 的 `name:`),不是 IP serial |
| beforeDevCommand cwd 不可靠 | 已移除,vite 由开发者手动起(5174);build 前手动 `npm run build` |
| `PluginInvokeError` 路径 | 在 `tauri::plugin::mobile::`,不在 `tauri::plugin::`(dev 分支文档与稳定版有差异,以本地 registry 源码为准) |
| Plugin 基类构造 | `abstract class Plugin(private val activity: Activity)`——参数是 Activity 非 Context |
| 插件 gradle JVM target | 必须与主工程对齐(17),Kotlin 插件默认 1.8 会 Inconsistent JVM-target 报错 |
| gen/android BuildTask.kt | 硬编码 node/tauri.js 路径(本机配置):无全局 tauri CLI 时 args 需前插项目内 tauri.js 绝对路径;换机器需改 |
| release APK 未签名 | `tauri android build --apk` 产 unsigned,用 build-tools 的 zipalign + apksigner + `~/.android/debug.keystore`(pass: android)签名后可安装 |
| 插件权限文件 | build.rs 的 `tauri_plugin::Builder::new(COMMANDS)` 自动生成 permissions/autogenerated(可入库) |
| 主仓库测试基线 | **246 lib + 1 repro = 247**(AGENTS.md 的 229 为旧数字);门控类改动前先实测记录基线 |

### 20.4 阶段 0+1 验证状态(2026-09-07)

- Rust 交叉编译:aarch64 release 通过(协议核心 + 插件);host check 0 警告
- 测试:主仓库 247 全绿(门控零回归);android 仓库 5 绿(检测 4 + settings 1)
- 真机(REDMI K80 Ultra / Android 16,无线调试):APK 安装启动成功,UI 渲染正常,detect_campus 端到端返回真实数据,无崩溃
- 待真机验证(需校园网环境):bind_to_wifi 实际绑定、登录/注销闭环、settings 持久化写盘

---

## 二十一、安卓端全功能对齐(2026-09-07,android 仓库 feat/android-full-parity)

> 用户需求"把 Windows 版全功能搬过去"。阶段 0+1 最小骨架升级为全功能镜像:命令名与桌面版对齐,前端整库复刻后裁剪桌面专属。

### 21.1 架构与命令面

- **命令名对齐桌面**(`do_login/do_logout/check_portal_status/check_campus_status/self_service 六命令/账号五命令/get_init_data/get_logs 系列/check_network_quality/start_latency_test/check_update/download_update/install_update/get_mirror_urls`),前端 tauriApi 接口面不变、组件零改动;`do_login` 参数改 `Option<String>` 兼容桌面契约(前端仅传 adapterName,凭据后端回退配置)。
- **协议核心继续单点**:`auth::protocol`(登录/注销)、`auth::portal::check_portal_full`(探测)、`self_service`(自助六协议)、`network::quality`(TCP/DNS/DoH/HTTPS 分段计时)、`network::subnet::is_same_subnet_18` 全部无 cfg 门控直接复用;quality 的 surge_ping ICMP 兜底安卓运行时静默失败,TCP 优先策略下不影响结果。
- **新增跨平台适配**:主仓库 `infra/logger.rs::get_log_dir` 加 `cfg(target_os="android")` 分支(安卓 current_exe 指向只读 APK 目录,日志落 app_data_dir/logs;2652473)。

### 21.2 关键决策

- **密码加密 = AndroidKeyStore AES-GCM**(替代 DPAPI):手写插件 `plugins/keystore`,`encrypt(明文)→base64(iv+ciphertext)`/`decrypt`;密钥别名 campus_login_master 首用自生成、不出安全硬件。Rust 侧 `CryptoBridge{encrypt,decrypt}` 抽象(host 测试注入 base64 假桥,真机 run_mobile_plugin 走插件);磁盘形态 `EncodedSettings{settings(密码字段清空), password_cipher, self_password_cipher}`——密码与配置分离,解密失败置空不阻塞配置加载。
- **掩码/回退语义与桌面 save_config 同构**:空串或 `***` 回退已存值,`clear_password/clear_self_password` 显式清除;`masked_for_display` 唯一掩码出口;`resolve_self_password` 自助密码同规则。配置读改写用 `tokio::sync::Mutex`(std MutexGuard 不能跨 await,编译期即报)。
- **生物识别验证门**:官方 tauri-plugin-biometric(前端 `authenticate(reason, {allowDeviceCredential:true})` 支持锁屏凭据兜底)→ `verify_biometric_identity` 记 TTL 600s(进程内 AtomicU64,桌面 identity.rs 语义移植,时钟回拨视为过期);bind/offline 设门、查询不设门、reveal 强制设门不受总开关影响(与桌面完全同构)。
- **后台监控 = Rust tokio 循环 + 前台服务纯保活**:业务逻辑(检测→探测→状态机→自动重登→emit)全在 Rust `monitor_loop`(协议不双实现);手写插件 `plugins/foreground-service` 只做常驻通知+WifiLock+PARTIAL_WAKE_LOCK(Android 14+ specialUse type + PROPERTY_SPECIAL_USE_FGS_SUBTYPE);`should_attempt_login` 纯函数六条件(在线/校园网/掉线重连不受自动登录开关管/启动即离线需 auto_login_on_preparation/重连上限 max_disconnect_reconnect/cooldown auto_login_cooldown_secs)。事件 `background-check-result`(字段对齐桌面子集)/`login-log`。
- **开机自启**:BOOT_COMPLETED receiver 默认 disabled,`set_boot_autostart` 经 PackageManager setComponentEnabledSetting 动态启停 + SharedPreferences 记忆;设置页 get/set_auto_launch 命令等价映射到它。
- **更新流**:check_update(version.json + ghfast.top/gh-proxy.com/ghproxy.net 三镜像降级 + 逐段数值版本比较)、download_update(域名白名单 8 个 + 500MB 上限 + 流式进度事件 200ms 节流,落 app_data_dir/update/)、install_update(路径限制 update/ 目录内,经 tauri-plugin-opener 系统安装器打开 APK,无静默安装;REQUEST_INSTALL_PACKAGES 权限)。
- **前端整库复刻**(96 文件,排除 *.test.*):9 面板全量(总览 5 卡/账号/自助服务/质量/测速/设置/日志/监控);tauriApi 手术三类——同名命令直调、等价替换(verifyWindowsIdentity→biometric、openExternal→opener、autoLaunch/notification→安卓命令)、桌面专属桩 `desktopOnly`(reject 走 useNetwork 既有 catch→toast"失败"提示)或空监听;App.tsx 去 min-w-[800px] + safe-area(刘海屏 env())+ 删 RightPanel(日志保留全页面板);DockNav 移除 network 项(安卓系统托管网络);manualChunks 分包对齐桌面(最大 chunk 430KB)。
- **get_init_data 补齐桌面全字段**(adapters=[]/gpuInfo=null/refreshRate=60 等):防前端 useInitialDataLoad 读 undefined 崩溃。
- **砍除清单(桌面专属,明示)**:托盘/全局快捷键/窗口控制/提权 helper/适配器枚举与禁用恢复/DHCP 续租与 MAC 重置/DNS+DoH 设置/双适配器/Windows Hello/msi 安装/自动退出倒计时/GPU 信息/WebView 心跳。

### 21.3 新增坑清单

| 坑 | 处置 |
|---|---|
| tauri-plugin-biometric 整 crate `#![cfg(mobile)]` | host cargo test 编译为空 crate,`init()` 引用报 E0425——插件 init 必须放 `#[cfg(mobile)]` 分支 |
| std MutexGuard 跨 await | `future cannot be sent between threads safely`——配置读改写锁换 `tokio::sync::Mutex` |
| tauri::command 宏生成物可见性 | 命令 fn 必须 pub(生成 `__cmd__` 宏 private,E0603);跨模块注册时同 mod 内 pub 即可 |
| UpdateInfo 缺 serde(default) | version.json 字段缺失即反序列化失败;全字段 default + struct derive Default |
| spawn_blocking 双层 Result | `??` 或 `let inner = ...?;` 再展开;忘写第二个 `?` 得 Result<Result<..>> |
| gradle 单测命令 | `JAVA_HOME=D:\Java\jdk-17.0.20.1+1`,`local.properties` 写 `sdk.dir=C:/Users/ik/AppData/Local/Android/Sdk`(正斜杠,反斜杠转义易错;已入 .gitignore) |
| vite 大 chunk 警告 | 复制桌面 manualChunks 分包(react/motion/gsap/radix 四 vendor chunk) |

### 21.4 验证状态(2026-09-07)

- Rust:host `cargo test` 35 用例全绿零警告(crypto round-trip/密文不落盘/回退语义/验证门 TTL/回拨/登录历史/账号消毒/监控状态机/版本比较/白名单);主仓库 247 全绿(logger 门控零回归)
- 前端:`tsc --noEmit` 全过;`vite build` 零警告(分包后最大 430KB)
- Kotlin:三插件编译通过(过程中修掉两个自身低级错误:0xCAMP 非法十六进制字面量、Plugin 基类 Context→Activity——后者是 20.3 已记录坑的复发)
- APK:`tauri android build --target aarch64` 成功产出 universal release APK+AAB;zipalign+apksigner(debug.keystore)签名 28.6MB
- **真机部署被阻**:手机无线调试断开(mDNS 缓存端口全部 ConnectionRefused),待用户重新开启无线调试后 adb connect 安装验证
- 直跑 gradlew assembleArm64Debug 会失败于 rustBuildArm64Debug:`android-studio-script` 模式需连顶层 tauri CLI 的 WebSocket(ConnectionRefused),必须走 `tauri android build` 顶层流程;BuildTask 的 Windows fallback 链会把真实异常替换为 ".bat 启动失败",排障时先看首个异常
- 待真机验证(需校园网环境+用户):登录/注销闭环、自助服务(生物识别门弹出)、监控开关(前台通知出现)、自启开关、主题切换、更新检查

---

## 二十二、移动 UI 适配版 + 功耗治理(2026-09-08,feat/android-full-parity)

> 用户反馈驱动:①手机 UI 需单独适配版(桌面缩放路线不合格);②实测平均功耗 3W 过高;③动画不能一刀切,要适配天玑/骁龙;④轮询质疑(实测证实用户判断正确)。

### 22.1 功耗结论(原理分析,执行期待实测分项验证)

| 源 | 量级 | 处置 |
|---|---|---|
| 屏幕(亮屏 120Hz) | 1~3W | 物理功耗,不可控;测量须分项剔除 |
| WebView 动画满帧(流体背景 blur/磁吸/呼吸灯) | 0.3~1W(亮屏) | 移动外壳重写后流体背景/磁吸已不存在;余下 gsap 常驻循环走智能帧率 |
| PARTIAL_WAKE_LOCK 常驻 | 0.05~0.3W(息屏主项) | **改事件驱动短持锁**:NetworkCallback(onAvailable/onLost/onCapabilitiesChanged)→ acquire(3000) 自动释放;轮询间隔保持 15s 不变(毫秒级探测均摊 <10mW) |
| WifiLock HIGH_PERF | 小 | 保留(省电断流影响登录) |

### 22.2 移动 UI 设计(风格延续桌面,交互专为安卓)

- **保留复用**:tauriApi 契约/stores/i18n/主题 CSS 变量/radix 基件/各业务面板组件(SelfService/Account/Quality/Settings/Log/SpeedTest/Monitor)。
- **重写**:App.tsx 移动外壳(轻 header=状态色点+图标入口 / 单列卡流 max-w-560 / 底部 5 tab=BottomNav);MobileDashboard(呼吸信号灯 hero=签名元素,StatusState 四态色,一键登录先 bindToWifi 再 doLogin,发光主按钮);MobileMore(chips 子页聚合 检测/测速/日志/设置)。
- **移除**:TitleBar/StatusBar/RightPanel/FluidBackground/DockNav(磁吸)/OnboardingWizard(首次无账号直达账号页)/SponsorCard/窗口最大化监听。bundle 主 chunk 430→363KB。
- 呼吸灯用 tailwind animate-ping(16px 级 opacity 合成,成本可忽略);framer-motion 仅面板切换(交互期,不限帧)。

### 22.3 智能帧率 + 芯片档位(动画降档不砍)

- **useDeviceProfile**(静态一次判定):WebGL `UNMASKED_RENDERER_WEBGL` → Adreno=骁龙 / Immortalis|Mali=天玑 / PowerVR=旧天玑;+ hardwareConcurrency/deviceMemory/reduced-motion → tier(high/mid/low)。K80 Ultra 天玑 9400(Immortalis-G925, 8 核)判 high。
- **useAdaptiveFramePace**(LTPO 式动态帧率):pointerdown/touchstart/wheel/keydown passive 监听 → 2s 活跃窗口;`gsap.ticker.fps()` 全局限帧——active 60fps(跟手)/idle 30fps(低端 24;呼吸发光周期 ≥2s,30fps 视觉无损)/document.hidden 时 rAF 停转自然暂停;面板切换 markInteraction() 程序性提帧。framer-motion 交互动画不限帧。
- CSS 微动画不纳入管理(成本可忽略);`blurScale` 档位保留(供后续若恢复流体背景类效果)。

### 22.4 查漏修补

- tauri_plugin_notification 注册(lib.rs,依赖早已在);启动后台检测前动态 import 请求 POST_NOTIFICATIONS 运行时权限(Android 13+)。
- tauri.conf.json CSP:null → 基线(default-src 'self'; connect-src ipc 桥; img-src data:; style-src unsafe-inline 供 tailwind/radix)。
- tauriApi 补 bindToWifi(安卓专属);i18n 补 nav.more 与 mobile.* 双语段。

### 22.5 坑补充

| 坑 | 处置 |
|---|---|
| gen/android/tauri.settings.gradle 由 tauri build 重生成 | 手写插件 include 会丢;单跑 gradle 编译插件需临时补 include(**路径用正斜杠**,Groovy 反斜杠转义易错);tauri.settings.gradle 已入 .gitignore?否——按生成物管理,build 后确认 |
| adb 无线调试端口随开关变化 | `adb kill-server && start-server` 后 mDNS 才收集新端口;带 (2)(3) 标记的是新发现;手机重启开关后旧端口全部 ConnectionRefused |
| vite build 前端产物 | tauri android build 不跑 beforeBuildCommand(已移除),构建前手动 `npx vite build` |

### 22.6 验证状态

- Rust 35 测试绿;Kotlin 编译过;tsc/vite build 零警告;APK 28.7MB 签名完成(/tmp/app-mobile-ui.apk)
- 浏览器逐页体检(2026-09-08,vite dev + index.html `__TAURI_INTERNALS__` mock + IAB 443×983 逻辑视口):9 视图(5 tab+4 chips)零横向溢出;视觉确认总览/账号/自助/质量/监控五页;产出三处修补(e020da8:账号页单列/监控卡标题 flex-wrap/引导表单单列)已截图复验;体检后 mock 已完整还原(git diff 干净)
- 坑:IAB guest 渲染进程对 CSS 动画时钟节流(card-enter 停在 opacity 0)、每 tab 仅首次截图成功、进程会崩溃重启——页面级验证一律"重开 tab→等入场动画→WAAPI finish() 跳末态→pause 无限动画→关 backdrop-filter→首截";真机不受影响,真机验证才是金标准
- 真机反馈九项修复(2026-09-08):UI 侧五项浏览器验证提交(038a79a:登录注销按钮移底导航上方/header 间距/竖版关于页 AboutDialogMobile/ThemeDialog 空函数接线/开关 thumb left-0 锚点+滚动条隐藏);notification:default capability 补齐(f79c3ae,监控启动被 ACL 静默拦截的根因);CSP style-src 被 Tauri nonce 改写致运行时内联 style 全拦(e06dc90,dangerousDisableAssetCspModification:["style-src"])
- 排查工具:debug APK 的 JS console.error 可见(logcat Tauri/Console tag);run-as 可读应用私有文件(debug 签名可调试);WebView CDP 远程调试需 devtools socket(debug 构建默认未开,不可用)
- 待真机(设备息屏后 MIUI 关无线调试服务,adb 频繁掉线;恢复连接后继续):⑤登录报"请输入"(候选:saveConfigDirect→Keystore encrypt 卡死/失败,isLoggingIn 卡 true 后续点击被 busy 挡)⑥密码无法保存(同候选)⑦质量命令错误 ⑧FGS 未启动(通知渠道已建、无 MonitorService ServiceRecord、无 crash 日志;logger 未在 files/ 建 logs 目录,app_data_dir 路径待 run-as 确认)
- 真机串测收尾(2026-09-08 下午):⑤⑥⑦⑧ 全部定位修复(c5cbc84/0003fd1)。**共同根因**:tauri CLI 每次构建重生成 tauri.settings.gradle 丢全部插件 include——五个插件 Kotlin 类(Keystore/MonitorService/NetworkBind/Biometric/Opener)从未进 APK;修复=主 settings.gradle+app/build.gradle.kts 持久声明(CLI 不覆盖,注意 projectDir 相对 gen/android 是三级上跳)。**第二层**:插件参数类 data class 缺 Jackson Creator(parseArgs 即 MismatchedInputException)——手写插件一律用官方 @InvokeArg+lateinit var 惯例。**第三层**:do_login user 参数不回退配置(仅 password 有)而安卓一键登录不传 user;running 标志在 start_monitor 前置位,失败残留致假"运行中"。日志工具:Rust 侧 eprintln→logcat RustStdoutStderr(release 也可见,排障主力);Rust 日志文件在 run-as 视角的 ./logs/(app_data_dir 不带 files 子目录);**gradle up-to-date 会吞 tauri build 的 APK 更新**(jniLibs 是 symlink,改 Rust 后 rm APK 再 assembleArm64Debug -x rustBuildArm64Debug 强制重打)
- 真机终验(release v7):登录协议链(user_len=8+密码回退密文)/密码密文落盘 52B/ForegroundService isForeground=true+campus_monitor 常驻通知/监控 tick 15s 真实执行(3 次检测)/九项 UI 全部生效;质量检测命令链通,真实延迟数据待校园网环境;功耗 batterystats 按用户要求跳过(用户自测前台均值 1.2W)
- 移动端第二批(2026-09-08 晚):总览页复用 auth/DashboardPanel 四卡(账号管理/在线信息/上网记录/质量明细)+编辑按钮(excludeCards 新 prop 排除 quickActions——DHCP 续租 Windows 专属);SelfOnlineCard 底部注销按钮(isAndroid 才渲染,desktop 注销仍在 DockNav);自助密码误触发验证门根因=自动刷新 effect 凭据判断含未落盘草稿——修复只认 selfPasswordSaved+pwdTouchedRef 会话守卫(动过密码框本会话不再自动弹验证);日志页 flex-wrap+消息 min-w-0;底栏去阴影。**安卓死开关清理**:安卓 Rust 无消费的 desktop 设置全部隐藏(autoExitAfterLogin/autoExitOnOnline/hiddenStart/minimizeToTray/defaultPanel/campusExitOnFail/campusCheckStartTime;autoLaunch 保留——monitor_loop.rs:131 有真实现);"非校园网不登录"无需新开关:run_check_once→should_attempt_login 已含 on_campus 判定。i18n zh/en 各 11 处 Windows 字样清零(Hello→生物识别/DPAPI→系统密钥)。判定手机端某设置是否死开关:grep 安卓 src-tauri 的 Settings 结构体+命令实现,只有 config_state.rs 有字段而无消费点=死开关(serde 丢弃不回读)
- 真机 v8/v8.1 实测(2026-09-08 晚,release 28.7MB 无线 adb):总览四卡+编辑模式+注销按钮+底栏无阴影+死开关消失+新文案全部真机过;在线信息卡真实拉取 2 台设备(掩码态)。**新坑:MIUI 大字体使 shrink-0 前置元素总宽超容器**——日志行消息被压一字宽+横向滚动条(内容"看似空白"),横向滑动取证后安卓构建改两行布局(f23c7d8,desktop 仍单行);**排查招**:adb 无法代按屏下指纹,BiometricPrompt 弹出后需用户实体指纹;验证门等待中按 BACK 会触发 MTK WebView gralloc freeBuffer native crash(平台图形栈,非应用逻辑;logcat -b crash 取证),验证门空闲等待稳定;真机截图缩放(screencap 924×2120 vs 屏幕 1280×2772),input tap 坐标要按比例换算
- 移动端第三批(2026-09-08 深夜,GitHub 调研后实施):**卡片编辑体系全量化**——DashboardPanel extraCards 注入机制(宿主卡进同一套增删/排序/持久化,CardId 泛化 string&{} 保留字面量提示,旧布局迁移自动补 start 位新卡),hero/监控卡重写为标准 AnimatedCard 结构传入;**编辑滚动修复**:整卡 touch-none+默认 dragListener 吞滚动→把手(GripVertical+useDragControls+dragListener=false),EditableCardItem 抽组件因 useDragControls 不能在 map 里调。**超级岛**(dev.mi.com/xiaomihyperos 官方文档+HyperIsland-ToolKit/Gramophone):第三方上岛=通知 extras `miui.focus.param`(param_v2 JSON:business/param_island.smallIslandArea/ticker/timeout),媒体通知自动上岛但仅限音视频场景,正式上岛需小米提报审核;AOSP 方向是 API36 ProgressStyle/promoted ongoing。**SoC 分级**:ro.soc.model/ro.soc.manufacturer(Android12+ CDD 强制,SM8650=骁龙8G3/MT6991=天玑9400)经 android_system_properties crate(仅 android target 编译——host 会因 dlclose 编译失败),大核=/sys/cpu*/cpufreq/cpuinfo_max_freq≥1.8GHz;facebook device-year-class 已归档,MEDIA_PERFORMANCE_CLASS 可用。**坑:host cargo test 在 android 仓库基线即失败**(mobile-only 插件权限 host 收集不全→"Permission biometric:default not found",且会把 gen/schemas 降级重写,跑完须 git checkout 恢复)——Rust 验证用 tauri android build;多设备 mDNS 残留条目用 adb -s 指定 serial;monkey 启动不可靠用 am start -n 包/.MainActivity
- 国产厂商岛态适配全景(2026-09-08,调研后**用户决策收敛**:删全部厂商私有 extras——miui.focus.param/superx/live 分支移除,统一 Android 标准协议:ongoing+Chronometer+CATEGORY_SERVICE,即 AOSP16 promoted ongoing 触发特征,compileSdk 36 已达标;调研结论存档——vivo 原子岛=notification.superx.*(键名源 imwangwang/vivo-service 反编译,白名单制,livebridge 实测 AOSP16 在 vivo 不工作),华为+魅族共用 notification.live.*(EMAS 指南+OrderAgents/Pinme 实证),OPPO ColorOS16 官方走标准 Live Updates 无私有 API(FluidCapsule 实证),荣耀纯白名单合作制;若未来需要岛态展示再按厂商通道逐家接入)
- 批次审查闭环(2026-09-09,主审通读 diff+deepseek-v4-flash 分身交叉复核):两个真 bug——①**通知 Chronometer 每 tick 归零**:notify 同 ID 重建通知时 setWhen(now) 使计时起点重置,运行时长永远<检测周期;修=companion 记 startAtMs(onCreate 赋/onDestroy 清),buildNotification 固定起点(28ba21b)。②**extra 卡删除后每次切 tab 复活**:AnimatePresence key=deferredTab 使面板重挂、useState 初始化的迁移逻辑重跑,把不在已存布局里的 start 位 extra 卡补回——存储上"旧布局没有"与"用户删了"不可区分,新增 campus-dashboard-extra-removed 黑名单(删记录/迁移豁免/重加清除)(2960fcb)。**审查方法论沉淀**:存储迁移类补卡逻辑必须区分"从未见过"与"显式删除",否则复活 bug;分身复核抓到"每次切 tab"频率升级(比"每次启动"严重)。收敛:get_soc_info 回传仅 tier+型号(9 字段仅 tier 被消费,删 ro.mi.os.version.name 小米属性残留);tier_of 骁龙8系逐型号(SM8250~SM8750 数组,SM8635=8sGen3 是中端不能按 SM86 前缀判旗舰;tier 2/3 帧率同为 30/60,只有误判 tier 1 才降档,精度要求其实很低);excludeCards 提模块常量(字面量数组使 DashboardPanel memo 失效)。v12 编译通过+无线 adb 覆盖安装真机;真机验证不替用户 tap(用户在用手机),UI 项留自测清单;glm5-3-flash 分身报 provider 未配置(builtin:bigmodel-start-plan),需查 ~/.zcode/agents 绑定
- 真机五项反馈修复(2026-09-09,v13):**自动登录无效/定时测试开了不测/后台检测设置无效三问题同一根因**——安卓缺桌面 startup.rs→watcher::run_startup_tasks 的启动恢复链路,循环只在用户手动切换时启动,重启即丢;修=lib.rs setup spawn run_startup_tasks(monitor_loop.rs,1.5s 延迟对齐桌面):enable_background_check→start_background_check、enable_latency_test→start_latency_test、否则一次首屏质量检测(emit network-quality-result,胶囊/质量页不再永远 pending)、auto_login_on_start→auto_login_on_start(凭据+probe on_campus 才登录,复用 protocol_cmds::run_login)(7534c7e)。**校园网判定"能到网关却判非校园网"**:原 on_campus=同/18子网&&Portal 可达,跨/18 AP 区段(10.2.0.x vs 网关 10.2.127.254)必挂;对齐桌面 campus_check 三层(SSID→子网→网关可达)的安卓版——无 SSID(netsh)且非 root 无 ICMP,用内部 Portal TCP 可达兜底(portal 内网地址,连通即在校);**副产品:后台监控掉线重登此前因 subnet 不中 on_campus 恒 false 而永不触发**(4e04fd4)。检测间隔热更新:MONITOR.desired_interval_ms,start 幂等分支刷新,循环逐 tick 对比重建 tokio interval(重建后首 tick 立即返回要吃掉)。UI:header 删应用名,原位嵌 NetworkQualityCapsule(桌面复刻组件本就在 android frontend,hover 弹层触屏无效,wrapper onClick 跳质量页,组件不动)(2737e3a)。**坑:vite build 必须在 tauri android build 之前单独跑**(CLI 不执行 beforeBuildCommand,否则 APK 里是旧 dist);桌面判定参考在 tauri-app/src-tauri/monitor/campus_check.rs(check_gateway ICMP 兜底安卓无对应)
- 迁移完整性深挖(2026-09-09,主审三层对比+deepseek 分身 67 工具调用复核,v14):**命令面 42 invoke 全对齐、面板裁剪(NetworkPanel=Windows 语义)合理**;挖出 5 P1。已修(v14):①注销保护期 MONITOR.logout_protected_until_ms(do_logout 置 60s/登录成功或在线清零,should_attempt_login 调用侧闸——纯函数签名不动测试不改);②run_check_once 补 cache_source_ip(后台链路首次重登空 IP);③自动登录 5 次熔断(桌面 PREP_LOGIN_MAX_FAILURES 同值,协议失败与执行失败都计数,do_login 手动成功清零);④BootReceiver best-effort startActivity(FlAG_ACTIVITY_NEW_TASK,MIUI 需后台弹出界面权限;**关键认知:FGS 只给通知壳,Rust 循环由 tauri Activity setup 驱动,开机不拉 Activity 监控就是死的**);⑤判定兜底层 campus_gateway TCP 优先于 portal(公网 portal 家宽误判边界);⑥首页开关间隔读现值。**待拍板大项**:install_update 的 opener open_path 对私有目录 file:// 必失败(Android7+ FileUriExposedException,需 FileProvider 改造+AboutDialogMobile 补安装 UI——Manifest 已有 provider 但 file_paths.xml 缺 files-path);APK 下载无 SHA256 校验(前端传了 checksum 被后端忽略,镜像投毒面);零系统通知(enable_notification 形同虚设,需 tauri-plugin-notification Rust 侧发送);无自动更新循环(桌面 24h+update-available 事件,安卓死监听正好可接活)。**死监听清单**(桌面事件安卓不 emit,合理裁剪):Adapters×4/AutoExit×2/CampusExit×2/ConfigChanged;onAutoLoginResult 也死(启动自动登录只发 login-log,如要 toast 补 emit)。**方法论:跨平台迁移后必做"桌面功能面×安卓消费面"矩阵对比**——启动恢复/注销保护/熔断/源IP缓存全是"桌面有、安卓精简版悄悄丢"的语义,靠调用点逐行对照才能发现
- 移动更新流闭环(2026-09-09,v15,迁移大项全部落地):①notify_system(monitor_loop.rs,tauri-plugin-notification Rust 侧 builder().show(),enable_notification 闸)挂掉线(was_online 翻转且 on_campus)/重登失败/达上限/自动登录结果;②start_update_check_loop(update_cmds.rs,30s 首查+24h 循环,UPDATE_LOOP_RUNNING 防重入)emit update-available 接活前端死监听+新版本通知;③SHA256=GitHub API release 资产 digest("sha256:hex" 服务端算,最可信)经 UPDATE_CHECKSUM 缓存,download_update 完成流式比对(sha2 crate),不匹配删文件;字段缺失/格式异常跳过(version.json 尚无 checksum);④installApk 插件命令=FileProvider.getUriForFile(filesDir/update/,authority=packageName.fileprovider)+ACTION_VIEW+FLAG_GRANT_READ_URI_PERMISSION,file_paths.xml 补 files-path,插件 build.rs COMMANDS+default.toml 加 installApk(schema 再生成提交);**check_update 增强拉 api.github.com releases/tags assets(.apk 过滤+digest)——version.json 无 assets,下载地址只能来自 API,API 失败降级空 assets 前端走外链**。AboutDialogMobile:一键下载(getMirrorUrls 4 源依次重试+onDownloadProgress 进度条)→installUpdate;release 无 .apk 资产时按钮退回外链。**发布流程新增要求:安卓版本 Release 必须上传 APK 资产(否则应用内更新不可用,只剩外链);GitHub 自动提供 digest 无需手写 .sha256**。坑:tauri 插件新命令权限=build.rs COMMANDS 数组+permissions/default.toml 两处,autogenerated 由 tauri_plugin::Builder 再生成;gen/schemas 随构建更新要一并提交
- 在线信息卡精简+质量卡转圈修复(2026-09-09,v20/v21,两端同步):①在线卡每行=IP+MAC(formatMac 破折号,与 IP 同行)+上线时间+行内注销(ConfirmDialog 确认→selfOfflineSession,密码空串回退;sessionId 原始 JSON 透传一直都有),去时长/流量/底部整体注销(安卓 isAndroid 分支随之删除)——对齐用户"自助服务在线列表精简"诉求;②60s 轮询(useSelfCardFetch refetch,hasCred 闸)使到期下线设备自动消失,此前只查一次;③质量卡转圈根因=展示门槛 `quality!=='unknown'`,增量推送期间 quality 停在 unknown/busy(胶囊修复后首次 busy 降 unknown 更加重此象),改 hasLatency 门槛(extract 双延迟任一≥0)+unknown/busy 按 getLatencyLevel 推断 Badge 等级,与胶囊同语义——真机 16:41"胶囊 21ms 而卡片转圈 40 分钟"即此根因;④extractErrorMessage 增强:插件 reject 普通对象(BiometricPrompt 取消 {code,message})走 String() 得 "[object Object]",优先取 message 字段(真机验证取消 toast"身份验证已取消")。**坑:冷启动落自助服务页→切入自动验证门→BiometricActivity 挂前台等指纹,adb screencap 全黑+圆圈,极易误判"启动死锁"**(Rust 侧 FGS/监控全正常,定位靠 dumpsys activity topResumedActivity);MIUI 左下角手势吞 tap+根页 BACK 退后台组合,adb 驱动切 tab 不可靠——验证布局类需求优先让冷启动直落目标页;掩码 reveal 无 TTL(验证门通过后保持到手动切回,"时间到了自动隐藏"仅指设备行)
- 安卓启动提速+小核功耗+60s 间隔(2026-09-09,v24,android 仓库):①run_startup_tasks 三链(后台检测/质量首测/自动登录)由串行 await 改并行 spawn——质量首测(12 域名数秒级)此前阻塞自动登录,1.5s 就绪窗口收窄到 500ms+auto_login probe 失败/非校园网 3s 重试一次;②cpu_affinity.rs:解析 /sys/devices/system/cpu/cpu*/cpufreq/cpuinfo_max_freq 按频率分簇(最低频簇=小核,频率差<30% 同构不绑),60s 一拍的 Portal 探测走 portal_probe_on_little_cores(专用短命线程绑小核+oneshot 回传;tokio worker/spawn_blocking 池共享不能直接绑,否则登录等前台任务被拖小核);绑核失败静默退 EAS;libc 加在 android target deps(依赖树内已有)。③后台检测默认间隔 15s→60s:config_schema_version 字段(Default=1,旧文件 serde 缺字段=0)触发 load_from 一次性迁移 15000→60000 并落盘——**不做 version 字段的迁移会死循环**(用户主动设回 15s 落盘后被反复覆盖);前端 constants/MobileDashboard fallback 同步。坑:Config/Settings 全字段字面量初始化(Default impl)加字段必须同步补,否则 E0063 只在 aarch64 交叉编译时暴露(host check 被 build script 权限问题挡住)
