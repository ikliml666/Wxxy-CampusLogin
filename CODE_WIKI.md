...
│           ├── network/             # 网络模块
│           │   ├── mod.rs           # 重导出
│           │   ├── client.rs        # 缓存基础设施 (NET_CACHE/HTTP客户端/TLS 1.3+回退)
...
│           │   └── session.rs       # 登录/注销会话管理 (full_login_inner/full_logout_inner)
│           ├── account/             # 账号模块
│           │   ├── mod.rs           # 多账号管理命令
...
│           ├── monitor/             # 监控模块
│           │   ├── mod.rs           # 重导出
│           │   ├── watcher.rs       # 后台检测调度器 (PortalCheckResult 职责分离)
...
│           │   ├── elevation.rs     # UAC 提权 (ShellExecuteW) + GUID 解析
│           │   ├── gpu.rs           # GPU 信息检测 (DXGI Win32 API) + 刷新率检测 + 浏览器参数
...
**两阶段检测**:
1. Phase 1: 并行测试网关 + DNS 服务器 + DoH 服务器 → 更新评分表 → 增量推送
2. Phase 2: 并行预解析 HTTPS 主机名 → 分批（每批4个）并行测试 HTTPS 网站 → 每批完成后增量推送
...
> **位置说明**: DNS 评分系统（`DNS_SERVER_SCORES`/`DOH_SERVER_SCORES`/`DnsServerScore`/`DohServerScore`）、评分更新/查询函数、DoH 解析函数（`resolve_via_doh`/`resolve_host_smart` 及辅助函数）均位于 `network/dns.rs`。HTTP 计时函数（`measure_https_timing`/`measure_dns_query`/`measure_doh_timing`）位于 `network/timing.rs`。
...
| `enable_doh_for_dns()` | 启用 DoH (COM ShellExecuteW 提权 `shell_exec_elevated` 优先，回退 `run_elevated`) |
| `setup_dns_doh()` | 一键设置推荐 DNS + DoH (netsh + COM ShellExecuteW 提权 `shell_exec_elevated` 优先，回退 `run_elevated`) |
...
- `checkOnline` 使用 epoch 计数器防竞态 + 并发锁，包含完整校园网检测逻辑（campusWifi/campusWired/a1OnCampus/a2OnCampus 等）
...
**openExternal 安全逻辑**: 协议白名单(http/https) + URL长度限制(2048) + URL解析验证
...
| 文件 | 说明 |
|------|------|
| `DashboardPanel.tsx` | 总览面板，卡片可重排（framer-motion Reorder.Group），3种子组件（QuickActionsCard/AccountManageCard/NetworkQualityCard） |
| `AboutDialog.tsx` | 关于对话框 |
...
| `MonitorPanel.tsx` | 监控面板 |
| `QualityPanel.tsx` | 网络质量面板 |
| `SpeedTestPanel.tsx` | 速度测试面板 (PanelName 包含 'speedtest'，NAV_ITEMS 包含测速入口) |
| `StatusBar.tsx` | 状态栏 (用户自助服务按钮 + 交互动画) |
| `LatencyComponents.tsx` | 延迟组件 |
| `LatencyTimeline.tsx` | 延迟时间线 |
| `NetworkQualityCapsule.tsx` | 网络质量胶囊 |
...
| 文件 | 说明 |
|------|------|
| `NetworkPanel.tsx` | 3个卡片（网络适配器列表/适配器设置/DNS优化），新增适配器启用/单适配器获取新IP |
| `useNetwork.ts` | 网络逻辑 Hook |
| `constants.ts` | 网络常量 |
| `types.ts` | 网络类型定义 (Adapter, DnsDohStatus, DnsServerInfo 等 10 个类型) |
| `index.ts` | 模块导出 |
...
| `constants.ts` | 设置常量 (NAV_ITEMS, ISP_OPTIONS, THEME_OPTIONS 等) |
| `types.ts` | 设置类型定义 (Config, AutoLaunchResult, InitData) |
| `index.ts` | 模块导出 |
...
| 文件 | 说明 |
|------|------|
| `LogPanel.tsx` | 日志面板 (自然流式布局，日志去重，无虚拟列表/批量缓冲) |
| `ErrorBoundary.tsx` | 错误边界 |
| `ConfirmDialog.tsx` | 确认对话框 |
| `FluidBackground.tsx` | 流体背景 |
| `AnimatedNumber.tsx` | 动画数字 |
...
| `ui-types.ts` | UI 类型定义 (StatusState, PanelName, ThemeName, LogType, GpuTier, GpuInfo, LogEntry, ToastMessage, AdapterDisabledWarningData, AutoExitCountdownData, SystemNotificationData, SaveConfigResult 等) |
| `ui-constants.ts` | UI 常量 (MAX_LOG_ENTRIES=300/APP_VERSION='2.2.7'/APP_NAME='校园网登录助手'/PASSWORD_MASK='***') |
| `index.ts` | 模块导出 |
...
| 文件 | 说明 |
|------|------|
| `DockNav.tsx` | 适配器选择浮层 + 注销按钮 (无线蓝色Wifi/有线绿色Cable图标, 200ms延迟关闭)，GSAP 磁吸效果（MAGNETIC_RANGE=80, MAX_SCALE=1.35, MAX_LIFT=-14），economy档禁用 |
| `RightPanel.tsx` | 右侧面板，适配器信息卡片（可展开/折叠），刷新适配器按钮 |
| `TitleBar.tsx` | 标题栏，语言切换按钮/更新可用指示器 |
...
| Hook | 说明 |
|------|------|
| `useAnimationProfile.ts` | 动画配置，AnimationTier(high/standard/economy) + AnimationProfile 17字段 |
| `useAsyncLock.ts` | 异步锁 |
| `useBreatheAnimation.ts` | 呼吸动画 |
| `useGlowAnimation.ts` | 发光动画 |
...
| `usePulseAnimation.ts` | 脉冲动画 |
| `useRipple.ts` | 涟漪效果 |
| `useStartupBoost.ts` | 启动加速，GSAP timeline 编排5个元素入场 |
...
| `color.ts` | HEX→HSL 颜色转换 |
| `latency.ts` | 延迟等级/颜色计算 (显式 borderBg)，新增 `getLatencyLevel()`/`extractGatewayLatency()`/`extractExternalLatency()` |
...
platform/
  ...
  ├── gpu.rs — GPU 信息检测 + 显示器刷新率检测 (EnumDisplaySettingsW) + 动态浏览器参数 (build_browser_args)
  │   [GpuInfo 新增 gpu_preference: u8 字段]
  └── autostart.rs — 开机自启
...
*文档版本: v2.2.7 | 基于代码版本: CampusLogin v2.2.7 | 更新日期: 2026-06-19*
