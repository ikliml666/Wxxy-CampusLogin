...
pub struct NetworkStatus {
    ...
    pub last_quality_check_time: ArcSwap<Instant>, // 上次网络质量检测时间，15秒冷却防重复
...
| 15 秒冷却机制 | `last_quality_check_time` 字段记录上次检测时间，冷却期内跳过本轮检测，防止短时间内重复执行 |
| 冷却期初始化修复 | `last_quality_check_time` 初始化为 `Instant::now() - 3600s`（1小时前），确保首次检测不被冷却期阻止 |
...
### 4.6 DNS 智能解析 — `network/timing.rs`
...
| 15 秒冷却 | 每轮检测前检查 `last_quality_check_time`，冷却期内跳过，防止短时间内重复检测 |
| 冷却期初始化修复 | `last_quality_check_time` 初始化为1小时前，首次检测可立即执行 |
...
### 5.8 日志面板 — `shared/LogPanel.tsx` (v2.2.5 更新)

- **移除虚拟化模式**：右侧日志面板改为自然流式布局，不再使用虚拟列表
- **日志去重**：相同内容的日志只更新时间戳，不重复添加条目
- **移除批量缓冲**：移除 `logBuffer` / `logFlushTimer` 批量缓冲机制，日志即时渲染
...
*文档版本: v2.2.5 | 基于代码版本: CampusLogin v2.2.5 | 更新日期: 2026-06-17*
