...
> **版本**: v2.2.6 | **架构**: Tauri 2 (Rust 后端 + React/TypeScript 前端)
> **目标平台**: Windows (x64)
...
│           │   ├── gpu.rs           # GPU 信息检测 (DXGI Win32 API)
...
2. **Setup 钩子**:
   ...
   - **5 秒保底 showWindow**：独立线程 5 秒后检查窗口可见性，不可见则强制 `window.show()` + `set_focus()`，防止前端初始化异常导致窗口永远隐藏
...
pub struct NetworkStatus {
    ...
    pub portal_failure_count: AtomicU32,           // Portal 请求连续失败计数，>=3 触发 DHCP 续租重置 MAC
}
...
| `campusCheckStartMinutes` | u16 | 480 | 校园网检测静默期截止时间（分钟数，480=8:00） |
...
**Portal 容错机制** (v2.2.5 新增):

`NetworkStatus.portal_failure_count: AtomicU32` 字段记录 Portal 请求连续失败次数。后台巡检中：

1. 每次主/副适配器 Portal 请求失败（`is_request_failed: true`）时，`portal_failure_count` 自增
2. 连续 3 次失败时触发 `dhcp_release_renew_all`（MAC 重置 + DHCP 续租），仅对校园网子网适配器生效
3. 触发后重置计数器为 0
4. Portal 检测恢复正常（`Success`）时，`swap(0)` 重置计数器并记录原值日志
...
| `start_campus_exit()` | 校园网验证不通过时：30s后最小化到托盘，再30s后强制退出 (受 `campus_exit_on_fail` 控制) |
| `cancel_campus_exit()` | 取消校园网退出流程 |
...
| 文件 | 说明 |
|------|------|
| `NetworkPanel.tsx` | DNS 优化卡片，一键优化按钮 |
...
| `ui-types.ts` | UI 类型定义 (StatusState, PanelName, ThemeName, LogType, GpuTier, GpuInfo 等) |
| `ui-constants.ts` | UI 常量 |
| `index.ts` | 模块导出 |
...
| 文件 | 说明 |
|------|------|
| `DockNav.tsx` | 适配器选择浮层 + 注销按钮 (无线蓝色Wifi/有线绿色Cable图标, 200ms延迟关闭) |
| `RightPanel.tsx` | 右侧面板 |
| `TitleBar.tsx` | 标题栏 |
...
- 默认语言中文，`safeStorage` 持久化语言偏好（替代 `localStorage`）
...
platform/
  ...
  ├── elevation.rs — ShellExecuteW UAC 提权 + GUID 解析
  ├── gpu.rs — GPU 信息检测
  └── autostart.rs — 开机自启
...
*文档版本: v2.2.6 | 基于代码版本: CampusLogin v2.2.6 | 更新日期: 2026-06-17*
