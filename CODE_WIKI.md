...
- **Hello 验证门拆分（2026-09-06 用户收紧）**：`selfServiceState.ts` 两套门——绑定卡 `useHelloGate`（firstFree→needVerify→verified，首次免验共用一次）；**自助服务面板 `useSelfServiceVerify` 每次操作都验证**（刷新/踢下线/上网记录查询，不首免不共用——可看设备可踢人，比绑定敏感）。reveal 依旧每次验证（`markGateVerified` 只解锁绑定门）；`SelfServicePanel.test.tsx` 锁"每次操作必须调验证、失败不执行"
- 单测：`offline_success_parsing`（success 判定 4 边界）；前端 vitest 4 用例（凭据联动禁用/表格格式化断言+每次验证/验证失败不查询/注销确认后移除行）
