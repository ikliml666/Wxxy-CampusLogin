...
- **网络质量检测启动延迟**：程序启动后延迟 3 秒再执行网络质量检测（后端延迟测试循环），避免网络未稳定时 HTTPS 测试延迟异常。
- **网络质量检测增量推送**：Phase 1（网关+DNS+DoH）完成后立即推送结果，Phase 2 HTTPS 每批完成后增量推送，前端逐步填充延迟数据，不再等待全部测试完成才展示。
