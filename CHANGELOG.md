...
- **网络质量检测增量推送**：Phase 1（网关+DNS+DoH）完成后立即推送结果到前端，HTTPS 批次每完成一批推送累计结果，前端逐步填充延迟数据，无需等待全部检测完成。
- **修复启动后15秒内无法执行网络质量检测**：`last_quality_check_time` 初始化为 `Instant::now()` 导致冷却机制从启动时即生效，首次检测被15秒冷却期阻止。现已初始化为1小时前，首次检测可立即执行。
- **移除前端网络质量事件防抖**：500ms 防抖过滤了增量推送的中间事件，导致前端只能显示最终完整结果。移除防抖后增量推送的 Phase 1 和 HTTPS 批次结果可立即更新 UI。
