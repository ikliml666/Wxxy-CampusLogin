...
> - [x] 第六波审查 Info-1 修复 → `frontend/src/lib/easing-config.ts:11` 去除 `EASING_60HZ` 冗余 export（外部 0 引用）；commit `b6340bd`；tsc --noEmit 0 错误
> - [x] 第六波审查 Info-2 评估 → `monitor/watcher.rs:9` re-export 保留不成立（审查员遗漏 `commands/background.rs:10` 调用点，删除需跨 2 文件且破坏门面统一性）
> - [x] 第七波 3 文件边界项简化 2 项（B2 + F4） → B2 删除 DefaultAdapterResolver zero-sized struct + traits.rs 文件 + service.rs 2 处调用改为直接调用 resolve_adapter_names；F4 删除 useIpc() 伪 hook + AboutDialog/NetworkPanel 改为直接 import tauriApiWithRetry；B6 跳过（extension trait 是惯用法）；cargo check 0 错误；tsc --noEmit 0 错误
