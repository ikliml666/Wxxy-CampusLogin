...
// 安卓无 adapters 概念(桌面遗留链路,列表恒空、getAdapters 必 reject),adapterIp
// 恒传空串——后端 check_portal_status 对空 IP 回退缓存源 IP/默认路由。
// 此前 checkOnline 走"解析失败判无网络"路径,启动自动登录/注销后状态点恒被
// 打灰(真机反馈:不手动登录不变绿、过一会又变灰)的直接根因即此。
...
export const useAuthStore = create<AuthStore>((set) => ({
  ...
  checkOnline: async (cfg, _adps) => {
    ...
    try {
      ...
      // Portal 状态查询(空 adapterIp:后端回退缓存源 IP/默认路由,见 queryPortalStatus 注释)
      const portalResult = await queryPortalStatus('')
      if (!portalResult.ok) {
