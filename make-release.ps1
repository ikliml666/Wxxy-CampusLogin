...
# CHANGELOG 按版本归档（AGENTS.md 必守约定 1）：发布时整份快照到本地 changelogs/（不入 git），
# 防止单文件无限膨胀；APK 无需 .sha256——安卓更新链路校验走 GitHub API digest + version.json 兜底
$changelogPath = Join-Path $root "CHANGELOG.md"
if (Test-Path $changelogPath) {
    $archiveDir = Join-Path $root "changelogs"
    New-Item -ItemType Directory -Path $archiveDir -Force | Out-Null
    Copy-Item $changelogPath -Destination (Join-Path $archiveDir "CHANGELOG_v$version.md") -Force
    Write-Host "  + changelogs/CHANGELOG_v$version.md（CHANGELOG 快照归档）" -ForegroundColor Green
}
