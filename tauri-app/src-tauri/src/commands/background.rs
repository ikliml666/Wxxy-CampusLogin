...
fn check_campus_network(config: &crate::config::Config, adapters: &[crate::network::Adapter]) -> (bool, Option<String>, String) {
...
pub fn trigger_background_check(_state: State<'_, AppState>, app_handle: AppHandle) -> Result<CommandResult, String> {
    ...
    tauri::async_runtime::spawn(async move {
        run_background_check(&app_h, manual_cancel).await;
    });
        ...
        if let Ok(adapters) = get_adapters_cached() {
            ...
            if let Some(a1) = adapters.iter().find(|a| a.name == adapter1_name) {
                if a1.ip.is_empty() {
                    ...
                    adapter_statuses.push(adapter_status_entry(&adapter1_name, &a1.ip, a1.wireless, a1_online, if a1_online { "已在线" } else { "未在线" }));
                }
            ...
            if config.dual_adapter && !adapter2_name.is_empty() {
                ...
                if let Some(a2) = adapters.iter().find(|a| a.name == adapter2_name) {
                    if a2.ip.is_empty() {
                        ...
                        adapter_statuses.push(adapter_status_entry(&adapter2_name, &a2.ip, a2.wireless, a2_online_state, if a2_online_state { "已在线" } else { "未在线" }));
                    }
                } else {
